...
public class HistoryTimerTask {
    ...
    public void startHistory() {
        try {
            ...
            //查询当前时间段有效的任务(type in (2)  is_del=0  state in（待启动) )
//            List<Task> tasks = taskDAO.getHistoryUpdateStateTask(Arrays.asList(Constants.TASK_STAT_WAITSTART, Constants.TASK_STAT_FAILEE));
            List<Task> tasks = taskDAO.getHistoryUpdateStateTask(Arrays.asList(Constants.TASK_STAT_WAITSTART));
            ...
            //开始分析的
            List<Task> runningList = tasks.stream().filter(j -> j.getState() != j.getNewState() && j.getNewState() == Constants.TASK_STAT_RUNNING).collect(Collectors.toList());
            //通知开始分析历史视频
...
//	        		queryCon.getF().put("reserved",Constants.CHANNEL_RESERVER_GB28181_2011.toString());//国标标识的通道
	            	...
	            	for (int j = 0; j < channelList.size() ; j++) {
	            		...
	            		try {
			                ...
			                historyVideoJO.put("deviceId",channel.getUuid());//通道的uuid
			                historyVideoJO.put("channelNo",channel.getChannelNo());//国标通道的20位国标码，字符串
